...
def transform_md_for_5ch(md_text: str, mapping: dict) -> str:
    ...
    mapping: {'1.': '1.', '1.1': '1.1', ...}
    chapter6 본문 안의 6.X 인용은 정규식으로 4.X로 일괄 변환.
    chapter7/8의 7.X / 8.X 인용도 정규식으로 5.X 자식 항목으로 변환은 매핑에 위임.
    """
    import re as _re
    out = md_text
    for old, new in mapping.items():
        out = out.replace(old, new)
    # chapter6 본문 안 인용: "6.5절", "6.3.3절", "6.5절" 등 → "4.5절", "4.3.3절"
    # 단, "# 제 6 장" 같은 헤딩은 이미 매핑에서 처리됐으므로 충돌 없음
    if "분석 결과" in out[:200]:  # chapter6에 한정
        out = _re.sub(r"(?<![\d\.])6\.(\d+)(\.\d+)?절", lambda m: f"4.{m.group(1)}{m.group(2) or ''}절", out)
        out = _re.sub(r"(표|그림)\s+6-", lambda m: f"{m.group(1)} 4-", out)
    return out
...
def main():
    ...
    add_merged_chapter(
        doc,
        [
            (DRAFT_DIR / "chapter3_data.md", ch3_map),
